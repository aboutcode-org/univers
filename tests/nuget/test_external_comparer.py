...
def test_NuGetVersionRangeWithGitCommit(verSpec, ver):
...
def test_NuGetVersionRangeWithGitCommitNotInRange(verSpec, ver):
    versionInfo = VersionRange(verSpec)
    version = NuGetVersion(ver)
    comparer = GitMetadataComparer()
...
def test_MixedVersionCompare(version1, version2):
...
def test_MixedVersionCompareNotEqual(version1, version2):
    semVer1 = NuGetVersion(version1)
    semVer2 = NuGetVersion(version2)
    comparer = GitMetadataComparer()
...
def test_DictionaryWithGitCommit(version1, version2):
    semVer1 = NuGetVersion(version1)
    semVer2 = NuGetVersion(version2)
    comparer = GitMetadataComparer()
