...
def test_ParseSemanticVersionStrict(versionString):
    semVer = SemanticVersion(versionString)
    assert versionString == semVer.normalized_string
...
def test_SemanticVersionStrictEquality(versionString):
...
def test_SemanticVersionStrictEqualityPreRelease(versionString):
...
def test_TryParseStrictReturnsFalseIfVersionIsNotStrictSemVer(version):
